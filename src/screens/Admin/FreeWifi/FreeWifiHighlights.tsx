...
import { Save, Image, Upload, Eye, EyeOff, Plus, Trash2, Edit2, X, ChevronLeft, ChevronRight, Palette, Type, AlignCenter, AlignLeft, AlignRight, Columns } from 'lucide-react';
import {
  getFreeWifiHighlights, createFreeWifiHighlight, updateFreeWifiHighlight, deleteFreeWifiHighlight,
  reorderFreeWifiHighlights, getFreeWifiSliderDesign, updateFreeWifiSliderDesign,
} from '../../../services/freewifiData';

// Slider templates with different layouts
const TEMPLATES = [
  { 
    id: 'classic-center', 
...
// Backend fields are snake_case; the app's local shape (and every prop below) is
// camelCase. Exported so the public page's HighlightsSlider can reuse the same mapping.
...
function SliderPreview({ highlights, design }) {
      ...
      case 'spotlight':
        return (
          <div className="relative z-10 h-full flex items-center justify-center px-8 overflow-hidden">
            {/* Soft glow behind the text, tinted to match the chosen text color */}
...
// Design Tab Component with Save/Cancel. Isolated from the parent (FreeWifiHighlights)
// so editing colors/etc only re-renders this component, not the whole highlights list.
function DesignTab({ highlights, onSaved }) {
  ...
  useEffect(() => {
    getFreeWifiSliderDesign().then(settings => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateFreeWifiSliderDesign(designToBackend(editDesign));
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-6">
      ...
      {/* Preview - uses editDesign for live preview */}
      <div>
...
export default function FreeWifiHighlights() {
  ...
  const [loading, setLoading] = useState(true);
  // Only for the always-visible top preview; the Design tab manages its own copy
  // (see DesignTab) and reports back here via onSaved so this preview stays in sync.
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteFreeWifiHighlight(id);
  ...
  const toggleActive = async (id) => {
    ...
    try {
      ...
      await updateFreeWifiHighlight(id, fd);
    } catch (err) {
      setHighlights(prev => prev.map(h => h.id === id ? { ...h, active: !nextActive } : h));
  ...
  const moveHighlight = async (index, dir) => {
    ...
    setHighlights(arr);
    try {
      await reorderFreeWifiHighlights(arr.map((h, i) => ({ id: h.id, order: i })));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Design Tab */}
      {activeTab === 'design' && (
        <DesignTab highlights={highlights} onSaved={setDesign} />
      )}
    </div>
  );
}
