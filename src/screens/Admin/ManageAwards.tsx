...
import { Plus, Trash2, Edit2, X, Save, Image, Upload, Link2, FileJson, ArrowUp, ArrowDown, LayoutGrid, List, GripVertical, Award, Eye, Columns3, Grid3x3, Clock, Rows3, Star, ArrowRight } from 'lucide-react';
import { getAwards, createAward, updateAward, deleteAward, importAwardsJson, reorderAwards } from '../../services/awards';
import { getKmsSettings, updateKmsSettings } from '../../services/settings';
import { AWARD_STYLES } from '../../lib/gridStyles';

const EMPTY_FORM = { title: '', issuer: '', year: new Date().getFullYear(), description: '', image: '', image_file: null };
...
export default function ManageAwards() {
  ...
  const fetchItems = async () => {
    try { setItems(await getAwards()); } catch (err) { console.error(err); } finally { setLoading(false); }
  };

  const resetForm = () => { setForm(EMPTY_FORM); setEditingId(null); setShowForm(false); setPreview(''); setImageMode('url'); };

  const startEdit = (item) => {
    setForm({ title: item.title, issuer: item.issuer || '', year: item.year, description: item.description || '', image: item.image || '', image_file: null });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append('title', form.title); fd.append('issuer', form.issuer); fd.append('year', form.year);
  ...
  function AwardItemCard({ item, index, variant }) {
    ...
    return (
      <div draggable onDragStart={() => handleDragStart(index)} onDragOver={handleDragOver} onDrop={() => handleDrop(index)}
        ...
        {item.image ? (
          <img src={item.image} alt={item.title} className={`${isCollage ? 'absolute inset-0 w-full h-full' : 'w-full h-40'} object-cover`} />
        ) : (
      ...
      {showImport && (
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-5 mb-6">
          <h3 className="font-semibold text-gray-900 dark:text-white text-sm mb-3 flex items-center gap-2">
            <FileJson size={16} className="text-emerald-600" /> Import Awards from JSON
          </h3>
          ...
          <textarea placeholder='[{"title": "...", "issuer": "...", "year": 2026, ...}]'
